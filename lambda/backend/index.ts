...
const ALLOWED_BUCKETS = new Set(
  (process.env.ALLOWED_BUCKETS ?? '').split(',').map((b) => b.trim()).filter(Boolean),
);
...
export const handler = async (
  ...
  if (!ALLOWED_BUCKETS.has(bucket)) return error(403, `Bucket "${bucket}" is not allowed`);
  ...
  try {
    ...
    console.error(`Failed to generate pre-signed URL: ${(err as Error).message}`);
